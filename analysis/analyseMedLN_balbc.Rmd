...
signGenes3 <- DEgenes %>% group_by(cluster) %>% top_n(avg_logFC, 10) %>% 
  mutate(labelNam=gsub("^.*\\.", "", gene)) %>% filter(labelNam != "1")

genes.to.label = c("Bmp1", "Bmp2", "Bmpr1a", "Bmpr1b", "Grem1", "Grem2",
...
```{r dotplot}
Idents(seurat) <- seurat$grp
genesSel <- data.frame(gene=rownames(seurat)) %>%
  mutate(labelNam=gsub("^.*\\.", "", gene)) %>% 
  filter(labelNam %in% genes.to.label)

DotPlot(seurat, features =  genesSel$gene, group.by= "grp") +
  RotatedAxis() +
...
write.table(DEgenes,
            file=paste0(basedir, "/data/BalbcMedLN_DEGenes.txt"),
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
