...
seurat$label_plus_treat <- as.factor(seurat$label_plus_treat)
Idents(seurat) <- seurat$label_plus_treat
...
template_ah <- c(
    ...
    "gapVecCol <- seq(2, length(levels(seurat$label_plus_treat)), by=2)",
...
```{r dotplot, fig.height=4, fig.width=3.8}

selGenesDat <- data.frame(gene=c("Bmp2", "Bmp4", "Bmpr1a", "Bmpr1b", "Bmpr2"))
...
DotPlot(seurat, assay="RNA", features = selGenes$ensID, scale =F,
        dot.min = 0, dot.scale = 3, scale.by = "size") +
  scale_color_viridis_c() +
  coord_flip() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_x_discrete(breaks=selGenes$ensID, labels=selGenes$gene) +
...
selGenesDat <- data.frame(gene=c("Bmp4", "Bmpr1a", "Id1", "Bmpr2"))
selGenes <- selGenesDat %>% 
  left_join(., genes, by="gene")

DotPlot(seurat, assay="RNA", features = selGenes$ensID, scale =F,
        dot.min = 0.05, dot.scale = 9, scale.by = "radius") +
  scale_color_viridis_c() +
  coord_flip() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_x_discrete(breaks=selGenes$ensID, labels=selGenes$gene) +
...
## Dotplot with selGenes only Fibroblasts
```{r dotplot only fibroblasts, fig.height=4, fig.width=3.8}

seuratSub <- subset(seurat, label == "Fibroblasts")
...
DotPlot(seuratSub, assay="RNA", features = selGenes$ensID, scale =F,
        dot.min = 0, dot.scale = 3, scale.by = "size") +
  scale_color_viridis_c() +
  coord_flip() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
selGenesDat <- data.frame(gene=c("Bmp4", "Bmpr1a", "Id1", "Bmpr2", "Id2"))
selGenes <- selGenesDat %>% 
  left_join(., genes, by="gene")
...
## Dotplot with selGenes label plus treatment
```{r dotplot label plus treatment, fig.height=6, fig.width=10}
...
ordDat <- expand.grid(levels(seurat$label), unique(seurat$treatment)) %>% 
  mutate(ord=paste0(Var1, "_", Var2)) %>% arrange(., Var1)
seurat$label_plus_treat <- factor(seurat$label_plus_treat, levels = ordDat$ord)
...
## vis Fibro + Macro genes
```{r plot sel fibro macro  genes, fig.height=14, fig.width=8}
