...
## cluster 1 (dying cells, a lot of ribosomal genes)
## cluster 17 (erythrocytes (Fech, Bpgm, Snca))
seurat <- subset(seurat, idents = c("1", "17"), invert=T)
...
seuratSub <- subset(seurat, integrated_snn_res.0.4 == "9")
...
Tcells <- colnames(seuratSub)[seuratSub$integrated_snn_res.0.1 %in% c("2", "4")]
NKcells <- colnames(seuratSub)[seuratSub$integrated_snn_res.0.1 %in% c("0", "1", "3")]
...
seurat$label <- "other"
seurat$label[which(seurat$integrated_snn_res.0.4 %in% c("4"))] <- "Macrophages"
seurat$label[which(seurat$integrated_snn_res.0.4 %in% c("12"))] <- "Bcells"
...
seurat$label[which(seurat$integrated_snn_res.0.4 %in% c("11"))] <- "pDCsDCs"
seurat$label[which(seurat$integrated_snn_res.0.4 %in% c("15"))] <- "proliferating"
seurat$label[which(seurat$integrated_snn_res.0.4 %in% c("0", "13"))] <-"Fibroblasts"
seurat$label[which(seurat$integrated_snn_res.0.4 %in% c("2", "19"))] <- "Cardiomyocytes"
seurat$label[which(seurat$integrated_snn_res.0.4 %in% c("3", "5"))] <- "Endothelial"
seurat$label[which(seurat$integrated_snn_res.0.4 %in% c("7", "16", "18"))] <- "Perivascular"
seurat$label[which(seurat$integrated_snn_res.0.4 %in% c("10"))] <- "endocardialEndothelial"
seurat$label[which(seurat$integrated_snn_res.0.4 %in% c("20"))] <- "capEndothelial"
seurat$label[which(seurat$integrated_snn_res.0.4 %in% c("8"))] <- "InfMonos"
