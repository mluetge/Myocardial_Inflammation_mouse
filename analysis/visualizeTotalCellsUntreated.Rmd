...
## expression level of cluster markers
```{r expression level of cluster markers}
...
## assay data
clusterAssigned <- as.data.frame(seurat$label) %>%
...
## genes of interest
genes <- data.frame(gene=rownames(seurat)) %>% 
  mutate(geneID=gsub("^.*\\.", "", gene)) %>% filter(geneID %in% selMarker$geneID)
...
logNormExpresDat <-logNormExpres %>% column_to_rownames("ident") %>% t(.) %>% 
  data.frame(.) %>% rownames_to_column(., var="gene")

 
write.table(logNormExpresDat,
            file=paste0(basedir, "/data/MuHeart_totalCells_",
                        "AvgCntsPerLab_Clustermarker.txt"), sep="\t", row.names=F,
            quote=F)
