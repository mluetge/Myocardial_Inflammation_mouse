...
## reorder int 
reordInt <- signIntDat %>% filter(cellA=="Tcells") %>% 
...
reordInt2 <- signIntDat %>%
  filter(cellA=="MacrophagesMonocytes" & cellB=="Fibroblasts") %>% 
  mutate(intA=gsub("_.*", "", intPair)) %>% 
  mutate(intB=gsub(".*_", "", intPair))
reordIntFin2 <- data.frame(intPair=paste0(reordInt2$intB, "_", reordInt2$intA),
                          CellsInt=paste0(reordInt2$cellB, "|", reordInt2$cellA),
                          mean=reordInt2$mean,
                          cellA=reordInt2$cellB,
                          cellB=reordInt2$cellA,
                          pVal=reordInt2$pVal,
                          Grp=reordInt2$Grp,
                          sign=reordInt2$sign)

signIntDatKeep <- signIntDat %>% 
  filter(cellA %in% c("Fibroblasts", "MacrophagesMonocytes")) %>% 
  filter(!cellB %in% c("Fibroblasts")) 


signIntDatOrd <- rbind(signIntDatKeep, reordIntFin, reordIntFin2) 
...
## int only in one grp
signIntDatOrdCtrl <- signIntDatOrd %>%filter(Grp=="control")
signIntDatOrdTCRM <- signIntDatOrd %>%filter(Grp=="TCRM")
