...
seurat <- subset(seurat, seqType == "sn")
seurat <- rerunSeurat3(seurat)
saveRDS(seurat, file = paste0(basedir,
                      "/data/MuHeart_Fibroblasts14D102plusIsotype12wk_snOnly", 
                      "_seurat.rds"))
...
## map signature on DM for each treatment {.tabset}
```{r map sign DM per treat cut 1.5}

template_dpt <- c(
    "### {{sig}}\n",
    "```{r map sign DM per treat cut 1.5 {{sig}},fig.height=3.5,fig.width=4.5, echo = FALSE}\n",
    "sign <- as.character('{{sig}}')",
    "signGenes <- allSign %>% dplyr::filter(signature == sign)",
    "pList <- NULL",
    "for(i in 1:length(unique(sce$treatment))){",
